...
def load_attendance():
    with engine.connect() as conn:
        query = sa.select(
            ...
            employees_table.c.id.label("emp_id"),
...
def show():
    ...
    if role == "user":
        ...
        st.markdown("### 👤 Chấm công của bạn")

        # ⚠️ mapping username -> employee
        emp = employees[employees["user_id"] == st.session_state.get("user_id")]

        if emp.empty:
            st.error("Không tìm thấy nhân viên tương ứng!")
            st.stop()
        ...
        if st.button("✅ Check-in"):
            ...
            if not today_record.empty:
                st.warning("Bạn đã check-in hôm nay rồi!")
            else:
            ...
            if today_record.empty:
                ...
                with engine.connect() as conn:
                    conn.execute(
                        ...
                        .where(attendance_table.c.id == int(today_record.iloc[0]["id"]))
                        .values(check_out=now)
                    )
                    conn.commit()

                st.success(f"Check-out lúc {now}")
                st.rerun()

        # ===== LỊCH SỬ =====
        st.markdown("### 📋 Lịch sử chấm công của bạn")

        user_df = df[df["emp_id"] == emp_id]
    ...
    if employees.empty:
        ...
        with st.form("add_attendance"):
            ...
            emp_name = st.selectbox("Nhân viên", employees["ho_ten"])

            work_date = st.date_input("Ngày", date.today())
    ...
    if role in ["admin", "hr"]:
        ...
        if not df.empty:
            ...
            new_checkin = st.text_input("Check In", selected["check_in"])
            new_checkout = st.text_input("Check Out", selected["check_out"])
